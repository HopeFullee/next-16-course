...
- ### 웹페이지 분석 플랫폼 (analytics platform)

  본 프로젝트에선 `"PostHog"`를 사용했다.

  ```
  웹 분석은 무엇인가?

  대표적으로 GA (Google Analytics)가 있고 말 그대로 웹 페이지 내 유저의
  모든 활동/상호작용 등을 분석/추적/기록 하는 도구이자 플랫폼이다.

  웹 분석을 왜 쓰는가?

  비즈니스 관점에선, 유저의 관심 범위를 파악한다. 이를 활용해 마케팅 전략으로써 사용할 수 있다.
  개발자 관점에선, 서비스 내 유저들이 만나는 "Error"를 추적하여 서비스를 보다 더 쉽게 관리할 수 있다.
  ```

  #### 특징:

  - `방문횟수 기록`: DAU, WAU, MAU 등의 웹 페이지 방문 횟수를 기록한다.

  - `이벤트 발생 구간 기록`: HeatMap을 통해 유저의 상호작용이 많거나 적은 위치를 기록함.

  - `Error 기록`: 웹 페이지 내 유저가 발견한 모든 "Error"를 세부적으로 ("when", "/where", "what typeOf Error" 등을) 기록함.
